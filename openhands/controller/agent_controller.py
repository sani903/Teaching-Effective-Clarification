    ...
    async def _handle_message_action(self, action: MessageAction) -> None:
        ...
        if action.source == EventSource.USER:
            ...
            if not self._first_user_message_processed:
                ...
                content_after_instance = re.sub(
                    r'^<instance>.*?</instance>\s*', '', text, flags=re.DOTALL
                )
                ...
                if self.preconditions_model:
                    ...
                    logger.info(f'Generated preconditions checklist: {checklist}')

                    # Send the checklist questions directly to the user as an agent message
                    questions_message = MessageAction(
                        content=checklist,
                        wait_for_response=True
                    )
                    self.event_stream.add_event(questions_message, EventSource.AGENT)

                    # Return early - don't proceed with RecallAction or normal flow yet
...
"""
    ...
    async def _update_state_with_augmented_task(
        self, augmented_task: MessageAction
    ) -> None:
        """
